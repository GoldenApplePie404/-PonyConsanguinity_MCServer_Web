...
3. **配置**
    - 请利用文本编辑器的搜索关键词将所有 `localhost:8000` 替换为您的服务器域名或 IP 地址，否则部分API将无法正常工作。
...
**安全效果**：
- 直接访问`http://localhost:8000/data/users.php`会返回403 Forbidden
...
**安全效果**：
- 防止目录列表泄露
- 访问`http://localhost:8000/data/`返回403错误
- 保护目录结构信息
...
- **前端页面**：`http://localhost:8000/pages/db_test.html`
- **后端API**：`http://localhost:8000/api/db_test.php`
